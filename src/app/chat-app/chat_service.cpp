...
public:
    ...
    void ReceiveSessionStartRequest(ServerReaderWriter<StreamResponse, StreamRequest> *stream,
         ...
         */
        auto f = sessions.find(username);
        if (f != sessions.end()) {
            sid = 0;
            status = 1;
        } else {
            sid = Session::GetSessionID();
            auto s = std::make_unique<Session>(sid, username, stream);
            sessions[username] = std::move(s);
            status = 0;
        }

        stream->Write(CreateSessonStartResponse(sid, status));
    }
